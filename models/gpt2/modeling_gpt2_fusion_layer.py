...
class FusionModel(GPT2PreTrainedModel):
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.mm_proj = MMProj(input_dim=self.mm_model.config.hidden_size, output_dim=config.hidden_size)
    ...
    def forward(
        ...
        gpt2_logits = self.lm_head(hidden_states)
        ...
        if pixel_values is not None:
            ...
            if input_ids.shape[1] > 1:
                ...
                vision_features = self.mm_model(pixel_values, output_hidden_states=True).hidden_states[-2][:, 1:, :]
        ...
        if scores is not None:
            lm_logits = lm_logits[:, -gpt2_logits.shape[1]:, :]
            lm_logits = scores.view(-1, 1, 1) * lm_logits + (1-scores).view(-1, 1, 1) * gpt2_logits.repeat(lm_logits.shape[0], 1, 1)
