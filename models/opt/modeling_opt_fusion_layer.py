...
class FusionDecoder(OPTPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`OPTDecoderLayer`]
...
class FusionModel(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.decoder = FusionDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        fusion_layer_config = OPTConfig.from_dict(config.to_dict())
    ...
    def forward(
        ...
        opt_logits = self.lm_head(outputs[0]).contiguous()
        ...
        if pixel_values is not None:
            ...
            if input_ids.shape[1] > 1:
                ...
                vision_features = self.mm_proj(selected_image_feature)
                ...
                inputs_embeds = torch.cat([vision_features, hidden_state], dim=1)
        ...
        if scores is not None:
            logits = logits[:, -opt_logits.shape[1]:, :]
            logits = scores.view(-1, 1, 1) * logits + (1-scores).view(-1, 1, 1) * opt_logits.repeat(logits.shape[0], 1, 1)
