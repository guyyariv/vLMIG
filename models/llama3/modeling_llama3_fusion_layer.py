...
class FusionModel(LlamaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`LlamaDecoderLayer`]
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        fusion_layer_config = LlamaConfig.from_dict(config.to_dict())
    ...
    def forward(
        ...
        llama_logits = self.lm_head(outputs[0])
        ...
        if scores is not None:
            logits = logits[:, -llama_logits.shape[1]:, :]
            logits = scores.view(-1, 1, 1) * logits + (1-scores).view(-1, 1, 1) * llama_logits.repeat(logits.shape[0], 1, 1)
