...
class FusionModel(GemmaPreTrainedModel):
    """
...
class CausalLMOutputWithPast(ModelOutput):
    ...
    fusion_key_values: Optional[Tuple[Tuple[torch.FloatTensor]]] = None


class MMProj(nn.Module):
    def __init__(self, input_size, output_size):
        super(MMProj, self).__init__()
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.mm_model = CLIPVisionModel.from_pretrained("openai/clip-vit-base-patch32")

        self.mm_proj = MMProj(input_size=self.mm_model.config.hidden_size, output_size=config.hidden_size)

        fusion_layer_config = GemmaConfig.from_dict(config.to_dict())
        fusion_layer_config.num_hidden_layers = 1
        self.fusion_layer = FusionModel(fusion_layer_config)
    ...
    def forward(
        ...
        fusion_key_values: Optional[torch.FloatTensor] = None,
        ...
        gemma_logits = self.lm_head(outputs[0])
        hidden_state = outputs[0]

        # 2. Merge text and images
        if pixel_values is not None:

            if pixel_values.shape[0] != hidden_state.shape[0]:
                hidden_state = hidden_state.repeat(pixel_values.shape[0], 1, 1)
            ...
            if input_ids.shape[1] > 1:
                ...
                image_outputs = self.mm_model(pixel_values, output_hidden_states=True)
                # this is not memory efficient at all (output_hidden_states=True) will save all the hidden stated.
                selected_image_feature = image_outputs.hidden_states[-2]

                selected_image_feature = selected_image_feature[:, 1:]

                image_features = self.mm_proj(selected_image_feature)
                fusion_input_ids = input_ids.clone()

                # Step 1: Find the index of 2 in each row
                index_of_two = (input_ids == 2).nonzero(as_tuple=True)[1]
                self.fusion_key_values = None
                # Step 2: Shift the '2' to the left by one position and replace the original position with 256000
                for i, row in enumerate(input_ids):
                    # Get the index for the '2' in this row
                    idx = index_of_two[i]
                    # Shift the '2' to the left if it's not at the beginning of the row
                    if idx > 0:
                        # Move '2' to the left
                        fusion_input_ids[i, idx - 1] = input_ids[i, idx]
                        # Replace the original place with 256000
                        fusion_input_ids[i, idx] = 256000

                inputs_embeds, attention_mask, labels, position_ids = self._merge_input_ids_with_image_features(
                    image_features, hidden_state, fusion_input_ids, attention_mask, labels
                )
            else:
                inputs_embeds = hidden_state
                attention_mask = torch.cat([attention_mask, torch.ones_like(attention_mask)], dim=1)[:,
                                 :self.fusion_key_values[0][0].shape[2] + 1]

            fusion_outputs = self.fusion_layer(
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=self.fusion_key_values,
                inputs_embeds=inputs_embeds,
                use_cache=use_cache,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
        ...
        if scores is not None:
            logits = logits[:, -gemma_logits.shape[1]:, :]
            logits = scores.view(-1, 1, 1) * logits + (1-scores).view(-1, 1, 1) * gemma_logits.repeat(logits.shape[0], 1, 1)
            logits = logits.mean(dim=0).unsqueeze(dim=0)
            if labels is not None:
                attention_mask = attention_mask[:1, -logits.shape[1]:]
        ...
        return CausalLMOutputWithPast(
            ...
            fusion_key_values=fusion_key_values,
        )

    def prepare_inputs_for_generation(
        self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, cache_position=None, pixel_values=None, scores=None, **kwargs
    ):
