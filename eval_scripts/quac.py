...
def evaluate(model_name, model, tokenizer, dataset, generate_images=False, k=1):
    if generate_images:
    ...
    for item in dataset:
        ...
        for i in range(len(item['questions'])):
            if item['orig_answers']['texts'][i] == 'CANNOTANSWER':
                continue
            ...
            answers = [item['orig_answers']['texts'][i]]
            if skip_flag:
                skip_flag = False
                continue
            skip_flag = False
            ...
            if generate_images:
              ...
              pixel_values = model.processor(images=images, return_tensors="pt").data['pixel_values'].to('cuda').to(
                  model.dtype)
              output_text_ids = model.generate(input_text, pixel_values=pixel_values, max_new_tokens=50, scores=scores)
            else:
              output_text_ids = model.generate(input_text, max_new_tokens=50)
            prediction = tokenizer.decode(output_text_ids[0], skip_special_tokens=True)[len(input_prompt):].strip().split('\n')[0]
            print(prediction)
